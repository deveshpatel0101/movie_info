...
class App extends Component {
  ...
  setScrollPosition = (path, scroll) => {
    this.setState((prevState) => ({
      scrollPosition: prevState.scrollPosition.concat({ path, scroll }),
    }));
  };
  ...
  getScrollPosition = (path) => {
    ...
    if (temp.length > 0 && temp[temp.length - 1].path === path) {
      ...
      this.setState((prevState) => ({
        scrollPosition: prevState.scrollPosition.slice(0, prevState.scrollPosition.length - 1),
      }));
    }
  };
  ...
  render() {
    return (
      <div className='App'>
        <BrowserRouter
          basename={
            window.location.host === 'deveshpatel.herokuapp.com' ||
            window.location.host === 'deveshpatel-staging.herokuapp.com'
              ? '/featured_works/movie_info'
              : '/'
          }
        >
          <Switch>
              ...
              render={() => (
                <Fragment>
                  <Header
                    ...
                    porgressBar={this.state.popular}
                    showInput={true}
                  />
                  ...
                  {this.state.popular || this.state.results ? (
                    this.state.query !== '' && this.state.results ? (
                      ...
                      <Results popular={this.state.popular} />
                    ) //display popular movies if query is not present
                  ) : null}
                </Fragment>
              )}
              exact={true}
