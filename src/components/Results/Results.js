        ...
        {result.poster_path || result.profile_path ? (
          <Link
            to={{
              ...
              search: `id=${result.id}&type=${
                result.media_type ? result.media_type : 'movie&sel=cast'
              }${
          ...
          {result.profile_path ? (
            <div className='person'>
                  ...
                  <Link
                    to={{
                      ...
                      search: `id=${result.id}&type=${result.media_type}${
                        result.media_type === 'person' ? '&sel=photos' : null
                      }`,
                    }}
                  >
                    {result.name}
              ...
              {result.popularity ? (
                <Typography variant='caption'>
                  <p>
                    <b>Popularity</b>: {result.popularity}
                  </p>
                </Typography>
              ) : null}
              ...
              {result.known_for ? (
                <Typography variant='caption'>
                  <p>
                    <b>Known For</b>:{' '}
                    {result.known_for.map((name, value, all) =>
                      all.length - 1 === value
                        ? name.title || name.original_name
                          ? `${name.title}`
                          : `${name.original_name}`
                        : name.title || name.original_name
                        ? `${name.title}, `
      ...
      <div className='grid'>
        {this.props.results
          ? this.props.results.map((result) =>
              result.poster_path || result.profile_path ? this.result(result) : null,
            )
